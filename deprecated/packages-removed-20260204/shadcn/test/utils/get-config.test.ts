...
test("get raw config", async () => {
  expect(
    await getRawConfig(
      path.resolve(import.meta.dirname, "../fixtures/config-none")
    )
  ).toEqual(null)

  expect(
    await getRawConfig(
      path.resolve(import.meta.dirname, "../fixtures/config-partial")
    )
  ).toEqual({
    style: "default",
    tailwind: {
  ...
  await expect(
    getRawConfig(
      path.resolve(import.meta.dirname, "../fixtures/config-invalid")
    )
...
test("get config", async () => {
  ...
  await expect(
    getConfig(path.resolve(import.meta.dirname, "../fixtures/config-invalid"))
  ).rejects.toThrowError()
  ...
  expect(
    ...
    resolvedPaths: {
      cwd: path.resolve(import.meta.dirname, "../fixtures/config-partial"),
  ...
  expect(
    await getConfig(
      path.resolve(import.meta.dirname, "../fixtures/config-full")
    )
  ).toEqual({
    style: "new-york",
    rsc: false,
    ...
    iconLibrary: "lucide",
    resolvedPaths: {
      cwd: path.resolve(import.meta.dirname, "../fixtures/config-full"),
      tailwindConfig: path.resolve(
        import.meta.dirname,
  ...
  expect(
    await getConfig(path.resolve(import.meta.dirname, "../fixtures/config-jsx"))
  ).toEqual({
    style: "default",
    tailwind: {
    ...
    iconLibrary: "radix",
    resolvedPaths: {
      cwd: path.resolve(import.meta.dirname, "../fixtures/config-jsx"),
      tailwindConfig: path.resolve(
        import.meta.dirname,
        "../fixtures/config-jsx",
        "tailwind.config.js"
      ...
      lib: path.resolve(import.meta.dirname, "../fixtures/config-jsx", "./lib"),
    },
    registries: {
      "@shadcn": "https://ui.shadcn.com/r/styles/{style}/{name}.json",
    },
