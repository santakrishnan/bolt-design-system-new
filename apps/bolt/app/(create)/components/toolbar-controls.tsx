  ...
  return (
    <Dialog onOpenChange={setOpen} open={open}>
      <DialogTrigger asChild>
        <Button className="hidden h-[31px] rounded-lg pl-2 md:flex" size="sm">
          <HugeiconsIcon
      ...
      <DialogContent className="dialog-ring min-w-0 overflow-hidden rounded-xl sm:max-w-md">
        ...
        <FieldGroup className="gap-3">
          <Field>
            <FieldLabel className="sr-only" htmlFor="template">
              Template
            </FieldLabel>
            <RadioGroup
            ...
            {Object.entries(commands).map(([key, cmd]) => {
              return (
                <TabsContent key={key} value={key}>
                  <div className="relative overflow-hidden border-border/50 border-t bg-surface px-3 py-3 text-surface-foreground">
